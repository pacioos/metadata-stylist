...
Silver Spring, MD 20910
 ...
 11/7/2011 john.maurer@hawaii.edu
    Avoid printing out unnecessary "Character_String" label for text fields.
-->
...
-->
<xsl:param name="g-tag-translation-URI" select="'utils/elements-iso.xml'"/>
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
        ...
        <xsl:variable name="thisIndent">
          <xsl:choose>
            <xsl:when test="$Character_String">
        ...
        <xsl:call-template name="printFormatted">
            ...
            <xsl:with-param name="optional-param-1" select="$thisIndent"/>
